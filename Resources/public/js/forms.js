;(function ($) {
		...
		this.each(function() {
			...
			$collection.updateChilds = function() {
				...
				$container.find('> div').each(function(index, child) {
					var $child = $(child);
					if($child.hasClass('image-container')) {
			...
			$collection.initChild = function($child) {
				if($child.hasClass('image-container')) {
					$child.find('.file-widget').fileWidget();
					var $file = $child.find('input:file');
					$child.find('.rename-widget').renameWidget({file: $file});
	...
	$(document).ready(function() {
		...
		/* Date picker */
		$('.form-wrapper input.date-picker').datepicker().on('changeDate', function (ev) {
            $(this).datepicker('hide');
        });
		
		/* Collections */
		$('.form-wrapper .collection-container').collectionWidget();
		
		/* Entities */
		$('.form-wrapper .entity_widget').entityWidget();
